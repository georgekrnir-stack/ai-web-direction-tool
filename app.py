    ...
    try:
        ...
        if available_models:
            for target in priority_targets:
                found = next((m for m in available_models if target in m), None)
                if found:
                    selected_model_name = found
                    break
        
        # 2. 見つからない、またはリスト取得失敗時はデフォルト
    ...
    with tab4:
        ...
        if user_input:
            if not model:
                ...
                history_text = "\n".join(st.session_state.chat_context[-10:]) 
